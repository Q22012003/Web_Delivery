...
function getNeighbors([r,c]){
  return [[0,1],[0,-1],[1,0],[-1,0]].map(([dr,dc])=>[r+dr,c+dc]).filter(([nr,nc])=>isValidCell(nr,nc));
}

function parseReserved(setIn){
  const nodes = new Set(), edges = new Set();
  for(const r of setIn){ r.includes("->") ? edges.add(r) : nodes.add(r); }
  return {nodes,edges};
}
...
export function findSafePathWithReturn(start, goal, reservedTimes, timeOffset=0, v1Path=[], v1StartTime=0, v2DelayTicks=17){
  if(!isValidCell(...start) || !isValidCell(...goal)) return null;
  const key = p => `${p[0]},${p[1]}`;
  const v1Offset = Math.round(v1StartTime/100);

  // --- PATH TO GOAL ---
  let pathToGoal=null, bestDelay=0;
  for(let delay=0; delay<=36; delay++){
    const delayed = shiftReserved(reservedTimes, delay);
    const candidate = aStar(start, goal, delayed, timeOffset+delay, v1Path, v1Offset, v2DelayTicks);
    if(candidate && candidate.length>=2){ pathToGoal=candidate; bestDelay=delay; break; }
  }
  ...
  if(v1Path && v1Path.length){
    for(let i=0;i<v1Path.length;i++){
      const p=v1Path[i]; const t=i;
      for(let dt=-2; dt<=2; dt++){
        const ti=t+dt; if(ti>=0) baseReserved.add(`${key(p)}@${ti}`);
      }
    }
    for(let i=0;i<v1Path.length-1;i++){
      const from=v1Path[i], to=v1Path[i+1]; const moveTime=i+1;
      for(let dt=-2; dt<=2; dt++){
        const ti=moveTime+dt; if(ti>=0) baseReserved.add(`${key(to)}->${key(from)}@${ti}`);
      }
    }
  }
  ...
  for(let waitAtHome=0; waitAtHome<=180; waitAtHome++){
    ...
    const candidate = aStar(goal, start, shifted, arrivalTime+waitAtHome+2, v1Path, v1Offset, v2DelayTicks);
    if(candidate && candidate.length>=2){ returnPath=candidate; break; }
  }
...
// ================== A* HEAD-ON SAFE ==================
function aStar(start, goal, reservedCombined, offset=0, v1Path=[], v1TimeOffset=0, v2DelayTicks=17){
  const {nodes:reservedNodes, edges:reservedEdges} = parseReserved(reservedCombined);
  const openSet=[], cameFrom=new Map(), gScore=new Map();
  const key = p => `${p[0]},${p[1]}`;
  ...
  while(openSet.length){
    ...
    if(pos[0]===goal[0] && pos[1]===goal[1]){
      ...
      return path.map(s=>{ const [rc]=s.split("@"); return rc.split(",").map(Number); });
    }
    ...
    if(posTime===0 && v1Path.length && v1Path[0][0]===pos[0] && v1Path[0][1]===pos[1]){
      const waitState=`${posK}@${posTime+1}`;
      if(!gScore.has(waitState)){
        gScore.set(waitState,curG+1);
        cameFrom.set(waitState,posState);
        openSet.push({pos:pos,f:curG+1+heuristic(pos,goal),time:posTime+1});
      }
      continue;
    }
    ...
    for(const nei of neighbors){
      ...
      while(anc){ const p=cameFrom.get(anc); if(!p) break; if(p===neiState){isAncestor=true; break;} anc=p; }
        ...
        for(let dt=-1; dt<=1; dt++){
          ...
          if(idx>=0 && idx<v1Path.length){
            ...
            if(idx>0){
              ...
              if(v1Prev && v1Prev[0]===nei[0] && v1Prev[1]===nei[1]) blocked=true;
              if(v1Prev && v1Prev[0]===pos[0] && v1Prev[1]===pos[1] &&
                 v1Pos[0]===pos[0] && v1Pos[1]===pos[1]) blocked=true;
            }
          }
        }
      }
      ...
      if(reservedNodes.has(`${nk}@${tentTime}`) || reservedNodes.has(`${nk}@${tentTime-1}`)) blocked=true;
      ...
      if(nk===key(goal) && v1Path.length){
        ...
        if(goalIdx>=0 && goalIdx<v1Path.length){
          ...
          if(v1AtGoal[0]===goal[0] && v1AtGoal[1]===goal[1]) blocked=true;
        }
      }

      // --- allow wait if neighbors blocked ---
      const freeNeighborExist = neighbors.some(n=>!reservedNodes.has(`${key(n)}@${tentTime}`));
      ...
      if(!gScore.has(neiState) || tentG<gScore.get(neiState)){
        ...
        const existing=openSet.find(o=>key(o.pos)===nk && o.time===tentTime);
        if(existing) existing.f=f; else openSet.push({pos:nei,f,time:tentTime});
      }
    }
